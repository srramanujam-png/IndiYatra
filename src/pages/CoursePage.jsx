...
const styles = `
  ...
  .level-tab {
    ...
    padding: 14px 24px; border-radius: 14px; border: 2px solid rgba(0,0,0,0.08);
  ...
  @media (max-width: 480px) {
    ...
    .level-tab-name { font-size: 1.0625rem; }
    .theme-row-title { font-size: 1.0625rem; }
    .theme-row-cta { padding: 6px 12px; font-size: 0.75rem; }
  }
`;
...
export default function CoursePage({ course, settings, completedLessons = new Set(), onThemeClick, onBack, onOpenSettings, onDashboard, onLikes, onBookmarks, onDiscover, onForYou, onAllCourses, onResume, bookmarks = new Set(), onToggleBookmark, userEditorialRole, onEditor, isAdmin, onAdmin, activePage, onSaveSettings, languages = [] }) {
        ...
        <>
          ...
          <div className="level-tabs">
            {levels.map((level, li) => {
              ...
              const prevLevel = li > 0 ? levelStats(levels[li - 1].level_id) : null;
              const isLocked = course?.sequential_unlock && li > 0
                && prevLevel.total > 0 && prevLevel.done < prevLevel.total;
              return (
                <div
                  key={level.level_id}
                ...
                >
                  <div className="level-tab-name" style={{ color: isLocked ? "#bbb" : lState === "complete" ? GREEN : meta.color }}>
                    {isLocked ? "🔒 " : lState === "complete" ? "✓ " : ""}{meta.label}
                  </div>
                  <div className="level-tab-classes">{meta.classes}</div>
                  <div className="level-tab-count" style={isActive && !isLocked ? { background: lState === "complete" ? GREEN : meta.color } : {}}>
                    {isLocked ? "Complete previous level" : lState === "complete" ? `${modCount} modules ✓` : lState === "progress" ? `${done}/${total} lessons` : `${modCount} modules`}
                  </div>
                </div>
              );
            })}
              ...
              : (
                <div>
                  {activeThemes.map((theme, ti) => {
                    ...
                    const prevTheme = ti > 0 ? themeStats(activeThemes[ti - 1].theme_id, selectedLevel) : null;
